...
def extract_roi_flow_segments_v19(base_dir, video_subdir, pre_frames, post_frames, frame_step):
        ...
        for filename in sampled_files:
            ...
            if frame_idx >= window_start:
                if frame_idx == window_start:
                    ...
                    global_dx = 0.6 * nose_dx + 0.2 * left_dx + 0.2 * right_dx
                    global_dy = 0.6 * nose_dy + 0.2 * left_dy + 0.2 * right_dy

                    # Face alignment: shift the crop box
                    align_iter = 0
                    while (global_dx ** 2 + global_dy ** 2) > 0.4:
                        align_iter += 1
                        if align_iter > 3:
                            break
            ...
            if frame_idx == window_end:
                ...
                segments_out = process_flow_segments(flow_region1_1, 1.4, 1.9, "left_eye", 1, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region1_2, 1.4, 1.9, "left_eye", 2, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region1_3, 1.4, 1.9, "left_eye", 3, frame_idx, crop_margin,
                                                     segments_out)

                segments_out = process_flow_segments(flow_region3_1, 1.4, 1.9, "right_eye", 1, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region3_2, 1.4, 1.9, "right_eye", 2, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region3_3, 1.4, 1.9, "right_eye", 3, frame_idx, crop_margin,
                                                     segments_out)

                segments_out = process_flow_segments(flow_region4_1, 1.4, 1.9, "mouth", 1, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region4_2, 1.4, 1.9, "mouth", 2, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region4_3, 1.4, 1.9, "mouth", 3, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region4_4, 1.4, 1.9, "mouth", 4, frame_idx, crop_margin,
                                                     segments_out)
                segments_out = process_flow_segments(flow_region4_5, 1.4, 1.9, "mouth", 5, frame_idx, crop_margin,
                                                     segments_out)

                segments_out = process_flow_segments(flow_region5_1, 1.4, 1.9, "nose", 1, frame_idx, crop_margin, segments_out)
                segments_out = process_flow_segments(flow_region5_2, 1.4, 1.9, "nose", 2, frame_idx, crop_margin, segments_out)

                segments_out = np.array(merge_overlapping_segments(segments_out, 0.25))
                segments_out = np.array(merge_overlapping_segments(segments_out, 0.25))
    ...
    filtered_segments = np.array(merge_overlapping_segments(filtered_segments, 0.25))
    filtered_segments = np.array(merge_overlapping_segments(filtered_segments, 0.25))
